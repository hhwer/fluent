...
void Mymat::getF(int N)
{
	...
	{
		double s1 = cos((1.0*(myorder[0]*size_n+k+0.5)/N)*M_PI);
		for(int j=0;j<size_m;j++)
		{
			double s2 = cos((1.0*(myorder[1]*size_m+j+0.5)/N)*M_PI);
			for(int i=0;i<size_l;i++)
			{
				ele[i+j*size_l+k*size_l*size_m] = 												s1*s2*sin((1.0*(myorder[2]*size_l+i+0.5)/N)*M_PI);
			}
		}
	}
}
